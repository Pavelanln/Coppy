...
static PyObject* THPStorage_shareDevice(PyObject* self, PyObject* noargs) {
  ...
  if (storage.data()) {
    ...
    at::globalContext().getAcceleratorHooksInterface().getIpcHandleSize(
        ipc_memory_handle_size, ipc_event_handle_size);
    ...
    at::globalContext().getAcceleratorHooksInterface().StorageShareDevice(
        storage,
        offset_bytes,
        new_memory_handle,
        new_event_handle,
    ...
    _handle = PyBytes_FromStringAndSize(
        new_memory_handle.get(), ipc_memory_handle_size);
    ...
    _event_handle = PyBytes_FromStringAndSize(
        new_event_handle.get(), ipc_event_handle_size);
...
static PyObject* THPStorage_releaseIPCCounter(
  ...
  try {
    ...
    at::globalContext().getAcceleratorHooksInterface().getIpcRefCounterFileSize(
...
static std::string THPStorage_bytesAsHandleString(
  ...
  if (PyBytes_AsStringAndSize(handle, &buffer, &handle_size) == -1) {
    TORCH_CHECK(
        handle_size == static_cast<Py_ssize_t>(ipc_handle_size),
        "incorrect handle");
  }
...
static PyObject* THPStorage_newSharedDevice(PyObject* _unused, PyObject* args) {
  ...
  std::string s_ipc_event_handle =
      THPStorage_bytesAsHandleString(_event_handle, ipc_event_handle_size);
  bool event_sync_required = PyObject_IsTrue(_event_sync_required);
  if (event_sync_required && s_ipc_event_handle.empty())
    return nullptr;

  std::string s_handle =
      THPStorage_bytesAsHandleString(_handle, ipc_memory_handle_size);
  std::string ref_counter_handle = PyBytes_AS_STRING(_ref_counter);
  ptrdiff_t ref_counter_offset =
      (ptrdiff_t)THPUtils_unpackLong(_ref_counter_offset);
