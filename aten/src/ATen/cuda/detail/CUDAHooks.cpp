...
void CUDAHooks::getIpcHandleSize(size_t& ipc_memory_handle_size,
  ...
  // ipc_memory_handle consists of handlel of type cudaIpcMemHandle_t and
  // two char type variables ShareableHandleType and SHAREABLE_HANDLE_VERSION
  ipc_memory_handle_size = CUDA_IPC_HANDLE_SIZE + 2 * sizeof(char);
...
void CUDAHooks::StorageShareDevice(const c10::Storage& storage,
  ...
  offset_bytes = shandle.offset;
  std::memcpy(new_memory_handle.get(), shandle.handle.c_str(), shandle.handle.size());
